...
foreach($tmp as $lang => $records) {
    ...
    for($i = 0; $i < $columns; $i++) {
        $column = array_column($records, $i);
        if(preg_match('/[a-z]+/i', $column[0])) {
            $output[$lang][$i] = ucwords(trim(str_replace(
                array('#', '-', 'php php', 'hhvm', 'php'),
                array('', ' ', 'PHP', 'HHVM', 'PHP'),
                $column[0]
            )));
        } else {
            $output[$lang][$i] = array_sum($column) / $num_records;
        }
    }
}
unset($column, $tmp);
